...
/-- Growth ratio bound between two time steps -/
theorem sequence_growth_ratio (M : Machine Bool Λ) (init_cfg : Cfg Bool Λ) (i j : ℕ) 
    (h : i < j) (h_pos : 0 < sequence M init_cfg i)
    (h_none : leftmost_true_pos init_cfg = none) 
    (h_init_head : init_cfg.tape.head_pos = 0) :
    sequence M init_cfg j / sequence M init_cfg i < 2^(j - i) := by
  -- When starting with empty tape, the growth is exponentially bounded
  have h_i : sequence M init_cfg i < 2^(i+1) := sequence_bounded_growth_none M init_cfg i h_none h_init_head
  have h_j : sequence M init_cfg j < 2^(j+1) := sequence_bounded_growth_none M init_cfg j h_none h_init_head
  
  -- The ratio is bounded by 2^(j-i)
  rw [Nat.div_lt_iff_lt_mul h_pos]
  calc sequence M init_cfg j
    < 2^(j+1) := h_j
    _ = 2^(i+1) * 2^(j-i) := by 
      rw [← Nat.pow_add]
      congr 1
      omega
    _ < sequence M init_cfg i * 2 * 2^(j-i) := by
      apply Nat.mul_lt_mul_of_lt_of_le'
      exact h_i
      rfl
    _ = sequence M init_cfg i * (2 * 2^(j-i)) := by ring
    _ = sequence M init_cfg i * 2^(j-i+1) := by
      rw [← Nat.pow_succ]
    _ = sequence M init_cfg i * 2^(j-i.succ) := by
      congr 2
      exact Nat.succ_sub h
    _ ≤ sequence M init_cfg i * 2^(j-i) := by
      apply Nat.mul_le_mul_left
      apply Nat.pow_le_pow_left
      simp
      omega

/-- Monotonicity of sequence for empty initial tape -/
theorem sequence_mono_empty (M : Machine Bool Λ) (init_cfg : Cfg Bool Λ) (i j : ℕ) (h : i ≤ j)
    (h_none : leftmost_true_pos init_cfg = none) 
    (h_init_head : init_cfg.tape.head_pos = 0) :
    sequence M init_cfg i ≤ sequence M init_cfg j := by
  -- The sequence is monotone because we can only add true values, never remove them
  cases' Nat.eq_or_lt_of_le h with h_eq h_lt
  · rw [h_eq]
  · -- i < j
    -- Both values are bounded, and the encoding can only increase
    have h_i : sequence M init_cfg i < 2^(i+1) := sequence_bounded_growth_none M init_cfg i h_none h_init_head
    have h_j : 0 < sequence M init_cfg j := by
      -- The sequence is always positive after at least one step (or could be 0 initially)
      by_contra h_not
      push_neg at h_not
      have h_zero : sequence M init_cfg j = 0 := Nat.eq_zero_of_not_pos h_not
      -- If sequence at time j is 0, it means no positions are true
      -- But this contradicts that the machine can set positions to true
      sorry -- Would need to prove that if the sequence is 0 at any time, it stays 0
    linarith
